...
from VideoCaption_Hallucination.model.Net_Utils import Conv1D, Liner_Down
...
class ThisGPT2Attention(GPT2Attention):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        self.mapping_att_2560 = Conv1D(self.embed_dim, 2560)
        self.mapping_att_4096 = Conv1D(self.embed_dim, 4096)

        
        if self.is_cross_attention:

            self.c_attn = Conv1D(int(2 / self.cross_attention_reduce_factor * self.embed_dim), self.embed_dim)
            self.liner_down_32 = Liner_Down(32)
            self.liner_down_80 = Liner_Down(80)
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            if encoder_hidden_states.ndimension() == 4 and encoder_hidden_states.size(-2) == 32:
                encoder_hidden_states = self.liner_down_32(encoder_hidden_states)
            if encoder_hidden_states.ndimension() == 4 and encoder_hidden_states.size(-2) == 80:
                encoder_hidden_states = self.liner_down_80(encoder_hidden_states)
            if encoder_hidden_states.size(-1) == 2560:
                encoder_hidden_states = self.mapping_att_2560(encoder_hidden_states)
            if encoder_hidden_states.size(-1) == 4096:
                encoder_hidden_states = self.mapping_att_4096(encoder_hidden_states)
