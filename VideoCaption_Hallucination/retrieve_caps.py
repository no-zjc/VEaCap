...
# vatex
v_annot_path = "/home/wy3/zjc_data/datasets/VATEX/vatex_caption.json"
v_fts_path = "/home/wy3/zjc_data/datasets/VATEX/vatex_clip_ViT_14_fts.hdf5"
v_path = "/home/wy3/zjc_data/datasets/VATEX/"
...
def load_msr_vtt_data(annot_path):
    ...
    v_mapping = File_Tools.get_vatex_mapping_dict()
    videos = []
...
def load_vatex_data(annot_path):
    annotations = json.load(open(annot_path))
    v_mapping = File_Tools.get_vatex_mapping_dict()
    videos = []
    for i, (key, value) in enumerate(annotations.items()):
        file_name = key + ".mp4"
        vid_int = int(v_mapping[key].split("video")[1])
        videos.append({'file_name': file_name, 'vid': vid_int, 'fts_key': key})
    return videos
...
def encode_captions(captions, model, device):
    with h5py.File('fts_files/encoded_captions_feature_768.h5', 'r') as f:
    ...
    #
    # with h5py.File('fts_files/encoded_captions_feature_768.h5', 'w') as f:
...
def main():
    ...
    dataset_type = ["msvd", "msr-vtt", "vatex"]
    dataset_str = dataset_type[2]
    print('Loading data')
    ...
    if dataset_str == "vatex":
        videos = load_vatex_data(v_annot_path)
        # xq_image_ids, encoded_images = encode_videos(videos, mv_fts_path, "msr-vtt")
        video_ids, video_frama_ids, fin_video_features = encode_videos_by_frame(videos, v_fts_path, "vatex")
    ...
    if dataset_str == "msr_vtt":
        json.dump(video_retrieved_caps, open(mv_path + 'msrvtt_combine_frame_retrieved_caps_Vit_B32_2.json', 'w'))
    if dataset_str == "vatex":
        json.dump(video_retrieved_caps, open(v_path + 'vatex_combine_frame_retrieved_caps_Vit_B32.json', 'w'))
