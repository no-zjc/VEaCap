...
from Main.THVC.config import Path
from Main.THVC.dictionary import Vocabulary
from Main.THVC.utils import Utils
from Main.THVC.evaluate import Evaluator
from Main.THVC.data import DataHandler
from Main.THVC.coaha import COAHA
...
from Main.THVC.config import ConfigTHVC
from Main.THVC.models.THVC.model import THVC
from Main.coco_caption.my_eval.eval_script import coco_evaluation_by_json
...
def evaluation_coaha(predict, dataset_str):
    ...
    path = Path(cfg, os.getcwd() + "/Main/THVC")
    # Vocabulary object
    voc = Vocabulary(cfg)
...
def evaluate_coaha_metric():
    ...
    if "msvd" in json_path or "MSVD" in json_path:
        ...
        result_text, evaluate_result_dict = coco_evaluation_by_json(json_path + "/MSVD_val_preds.json",
                                                                    "/home/wy3/zjc_data/PyProject/zero-shot-video-to-text/Main/coco_caption/my_eval/data/lable_references/msvd_video_caption_format.json",
                                                                    epoch)
        ...
        result_text, evaluate_result_dict = coco_evaluation_by_json(json_path + "/MSR-VTT_val_preds.json",
                                                                    "/home/wy3/zjc_data/PyProject/zero-shot-video-to-text/Main/coco_caption/my_eval/data/lable_references/msrvtt_caption.json",
                                                                    epoch)
...
def create_coaha_test(dataset_name, eval_label="test"):
    warnings.filterwarnings('ignore')
    # set seed for reproducibility
    utils = Utils()
    utils.set_seed(1)
...
def evaluate_metric_one(checkpoint_path, epoch, disable_rag, coaha_test, result_file_label=""):
    ...
    elif "msr-vtt" in checkpoint_path or "MSR-VTT" in checkpoint_path:
    ...
    elif "vatex" in checkpoint_path or "VATEX" in checkpoint_path:
        if os.path.exists(checkpoint_path + "/VATEX_val_preds" + result_file_label + ".json"):
            print("检测到当前模型已经存在测试结果文件！路径为：" + checkpoint_path + "/VATEX_val_preds" + result_file_label + ".json")
            Attention_Visualization_Tool.attention_write_label = 0
        else:
            execution_time_str = infer_vatex(checkpoint_path, "VATEX_val_preds" + result_file_label + ".json", disable_rag)
        # execution_time_str = infer_msrvtt(checkpoint_path, "MSR-VTT_val_preds" + result_file_label + ".json", disable_rag)

        result_text, evaluate_result_dict = coco_evaluation_by_json(checkpoint_path + "/VATEX_val_preds" + result_file_label + ".json",
                                                                    "/home/wy3/zjc_data/PyProject/zero-shot-video-to-text/Main/coco_caption/my_eval/data/lable_references/vatex_caption.json",
                                                                    epoch)
    ...
    if (coaha_test is not None) and ("vatex" not in checkpoint_path) and ("VATEX" not in checkpoint_path):
        coaha_test.evaluate(pre_dict)
...
def evaluate_more(result_path, epoch_batch_num, start_epoch, stride, epoch_num, dataset_name, result_file_label=""):
    ...
    if 'train' in result_file_label:
        eval_label = "train"
    else:
        eval_label = "test"
    if COAHA_Load and not dataset_name == 'vatex':
        coaha_test = create_coaha_test(dataset_name, eval_label)
    else:
        coaha_test = None
    for epoch in range(start_epoch, epoch_num+1, stride):
        ...
        if os.path.exists(checkpoint_path + "/evaluate_result" + result_file_label + ".txt") and not re_evaluate:
            ...
            ev = open(checkpoint_path + "/evaluate_result" + result_file_label + ".txt", 'r').read()
            print(ev)

        evaluate_metric_one(checkpoint_path, epoch, disable_rag, coaha_test, result_file_label)

def format_Contrast_model_result_evaluate(result_path, output_file, dataset_name):
...
if __name__ == '__main__':
    ...
    dataset_name = "msrvtt"
    epoch_batch_num = 8142
    # epoch_batch_num = 4071
    start_epoch = 6
    stride = 1
    epoch_num = 20
    ...
    if "msvd" in result_path or "MSVD" in result_path:
        ...
        epoch_batch_num = 1525
        # epoch_batch_num = 12194
        start_epoch = 10
        stride = 2
        epoch_num = 20

    if "vatex" in result_path or "VATEX" in result_path:
        dataset_name = "vatex"
        epoch_batch_num = 3908
        start_epoch = 15
        stride = 2
        epoch_num = 40


    result_file_label = "_0"
