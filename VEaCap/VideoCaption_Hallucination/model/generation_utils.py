...
# 1743
import inspect
from VideoCaption_Hallucination.model.Net_Utils import Generation_Tool
import warnings
...
import VideoCaption_Hallucination.model.dataset_create.hu_dataset_tool as HDT
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            cur_word_token_len = 1
            # argmax
            next_tokens = torch.argmax(next_tokens_scores, dim=-1)
            next_tokens_length = 1
            attention_write_out_1 = ""
            attention_write_out_2 = ""
            cur_word_1 = ""
            cur_word_2 = ""
            ...
            if hallucination_model is not None:
                ...
                infer_word_list = tokenizer_gpt2.batch_decode([infer_token])
                attention_write_out_1 = infer_word_list[0]

                infer_word_list = infer_word_list[0].split(" ")

                infer_word = " ".join(infer_word_list)
                # infer_word = tokenizer_gpt2.batch_decode([infer_token])
                # print(infer_word)

                detection_threshold = 0.8
                pre_word = infer_word_list[:-1]
                cur_word = infer_word_list[-1]
                cur_word_1 = cur_word
                cur_word_2 = cur_word
                cur_word_token_len = Generation_Tool.get_cur_word_token_length(tokenizer_gpt2, infer_token, cur_word)

                hi_bs = 10
                sg_top_k = 20

                # 添加方法获取最后一个词语的类型
                # 0,1,2  结构性词语，物体类别，动作/关系类别
                cur_word_label = Generation_Tool.get_word_type(cur_word)
                cur_hallucination_type = 0

                llm_candidate_words = tokenizer_gpt2.batch_decode(top_k_next_tokens[0])
                llm_candidate_words = [[s.replace(' ', '')] for s in llm_candidate_words]
                ignore_word_list = ['am', 'is', 'are', 'was', 'were', 'being', 'been', 'a', 'an', 's', 'two', 'up', 'the', 'some', 'with', 'his', 'her', '.']

                if cur_word_label != 0 and cur_word not in ignore_word_list and not cur_word.endswith('.'):
                    infer_word_token = clip.tokenize(infer_word).to('cuda')
                    infer_word_clip = clip_model.encode_text(infer_word_token)
                    concatenated_feature = torch.cat((infer_word_clip, encoder_output), dim=0)
                    input_feature = torch.stack([concatenated_feature], dim=0)
                    hallucination_rate = hallucination_model(input_feature)
                    if hallucination_rate[0][1] > detection_threshold or hallucination_rate[0][2] > detection_threshold:
                        if hallucination_rate[0][1] > detection_threshold:
                            cur_hallucination_type = 1
                        if hallucination_rate[0][2] > detection_threshold:
                    ...
                    if cur_word_label == cur_hallucination_type:
                        ...
                        for cw in candidate_words:
                            ...
                            if text == "":
                                ...
                                cur_text = cur_text + " " + ' '.join(cw)
                            hi_word_token = clip.tokenize(cur_text).to('cuda')
                            hi_word_clip = clip_model.encode_text(hi_word_token)
                            video_caption_cos = HDT.compute_cosine_similarity(encoder_output, hi_word_clip)
                            concatenated_hi_feature = torch.cat((hi_word_clip, encoder_output), dim=0)
                            identity_feature.append(concatenated_hi_feature)
                            cos_value.append(video_caption_cos)

                        # 遍历验证所有候选词语的幻觉率
                        hi_input_feature = torch.stack(identity_feature, dim=0)
                        result_list = []
                        for i in range(0, hi_input_feature.size(0), hi_bs):
                            test_inputs = hi_input_feature[i:i + hi_bs].to('cuda')
                            # 进行预测
                            with torch.no_grad():
                                hallucination_model.eval()
                                test_outputs = hallucination_model(test_inputs)
                                outputs = test_outputs.cpu().tolist()
                                for it in outputs:
                                    result_list.append(it)

                        result_list = HDT.reweight_result_by_cos(result_list, cos_value)

                        # 找到top_k的最小幻觉候选下标
                        top_k_hi_num = 10
                        if len(result_list) < top_k_hi_num:
                            top_k_hi_num = len(result_list)
                        top_k_hi_indices, top_k_hi_listinfo = Generation_Tool.get_max_no_hallucination(result_list, top_k_hi_num)
                        for i in range(len(top_k_hi_indices)):
                            cur_word = ' '.join(candidate_words[top_k_hi_indices[i]])
                            if cur_word in infer_word:
                                continue
                            else:
                                cur_word = ' ' + cur_word
                                break

                        next_tokens = torch.tensor(tokenizer_gpt2.encode(cur_word, add_special_tokens=False)).to('cuda')
                        next_tokens_length = next_tokens.shape[0]
                        cur_word_2 = cur_word
                        if cur_word_token_len > 1:
                            last_index = -1 * (cur_word_token_len - 1)
            ...
            infer_token2 = input_ids[0][prefix_id:]
            infer_word_list2 = tokenizer_gpt2.batch_decode([infer_token2])
            attention_write_out_2 = infer_word_list2[0]
            if Attention_Visualization_Tool.attention_write_label == 1:
                Attention_Visualization_Tool.attention_write_dict.update({"out_1": attention_write_out_1})
                Attention_Visualization_Tool.attention_write_dict.update({"out_2": attention_write_out_2})
                Attention_Visualization_Tool.attention_write_dict.update({"cur_word_1": cur_word_1})
                Attention_Visualization_Tool.attention_write_dict.update({"cur_word_2": cur_word_2})
                Attention_Visualization_Tool.attention_write_all_list.append(Attention_Visualization_Tool.attention_write_dict)

            model_kwargs = self._update_model_kwargs_for_generation(
                outputs, model_kwargs, is_encoder_decoder=self.config.is_encoder_decoder
            )
            cur_len = cur_len + next_tokens_length + 1 - cur_word_token_len
