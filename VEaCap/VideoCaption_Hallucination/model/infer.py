...
from VideoCaption_Hallucination.Attention_Visualization_Tool import *
...
def evaluate_model_bz_one(args, videoUtils, feature_extractor, tokenizer, model, eval_df):
    ...
    for idx in tqdm(range(len(eval_df))):
        ...
        if features is not None:
            ...
            # if args.features_type in ["lavit_frame_", "blip2_output_frame_", "blip2_output_fc_frame_"]:
            encoder_outputs = VideoUtils.get_all_features_by_json_files(args.features_type, fts_key, features_path)
...
def main(args):
    ...
    if args.dataset_type == "MSVD":
        ...
        if "train" in args.outfile_name:
            eval_df = pd.DataFrame(data['train'])
    if args.dataset_type == "MSR-VTT":
        data = videoUtils.load_data_for_inference_MSR_VTT(args.annotations_path, args.captions_path)
        eval_df = pd.DataFrame(data['test'])
        if "train" in args.outfile_name:
            eval_df = pd.DataFrame(data['train'])
    if args.dataset_type == "VATEX":
        data = videoUtils.load_data_for_VATEX(args.annotations_path, args.split_dict, args.captions_path)
        eval_df = pd.DataFrame(data['test'])
        if "train" in args.outfile_name:
            eval_df = pd.DataFrame(data['train'])
...
def infer_msvd(checkpoint_path, output_filename, disable_rag=True):
    ...
    parser.add_argument("--features_path", type=str, default="/home/wy3/zjc_data/datasets/MSVD-QA/msvd_video_clip_ViT_L_14_fts_four.hdf5")
    # parser.add_argument("--features_path", type=str, default="/home2/wy3/MSVD-QA/key_images/")
    # parser.add_argument("--features_type", type=str, default="blip2_output_frame_")
    parser.add_argument("--features_type", type=str, default="")
...
def infer_msrvtt(checkpoint_path, output_filename, disable_rag=True):
    ...
    parser.add_argument("--features_path", type=str, default="/home/wy3/zjc_data/zjc/MSR-VTT/key_images/")
    parser.add_argument("--features_type", type=str, default="blip2_output_frame_")
...
def infer_vatex(checkpoint_path, output_filename, disable_rag=True):
    start_time = time.time()
    parser = argparse.ArgumentParser(description='Model infer')

    # vatex config
    parser.add_argument("--dataset_type", type=str, default="VATEX")
    parser.add_argument("--features_path", type=str, default="/home/wy3/zjc_data/datasets/VATEX/vatex_clip_ViT_14_fts.hdf5")
    parser.add_argument("--data_root_path", type=str, default="/home/wy3/zjc_data/datasets/VATEX/")
    parser.add_argument("--annotations_path", type=str, default="/home/wy3/zjc_data/datasets/VATEX/vatex_caption.json")
    parser.add_argument("--captions_path", type=str, default="/home/wy3/zjc_data/datasets/VATEX/vatex_combine_frame_retrieved_caps_Vit_B32.json", help="JSON file with retrieved captions")
    parser.add_argument("--sg_path", type=str, default="/home/wy3/zjc_data/datasets/VATEX/SG/20/")
    parser.add_argument("--mapping_file", type=str, default="/home/wy3/zjc_data/datasets/VATEX/vatex_mapping.txt")
    parser.add_argument("--split_dict", type=str, default="/home/wy3/zjc_data/datasets/VATEX/split_dict.json")
    parser.add_argument("--hallucination_model_path", type=str, default="/home/wy3/zjc_data/PyProject/zero-shot-video-to-text/VideoCaption_Hallucination/model/dataset_create/vatex_hallucination_identity_001.pt")
    parser.add_argument("--video_dir", type=str, default="/home/wy3/zjc_data/datasets/VATEX/all_videos/")
    parser.add_argument("--checkpoint_path", type=str, default=checkpoint_path, help="Path to checkpoint to use for inference; If not specified, will infer with all checkpoints")
    parser.add_argument("--experiments_dir", type=str, default="/home/wy3/zjc_data/PyProject/zero-shot-video-to-text/result/",help="Directory where trained models will be saved")
    parser.add_argument("--model_path", type=str, default="/home/wy3/zjc_data/PyProject/zero-shot-video-to-text/result", help="Path to model to use for inference")
    parser.add_argument("--infer_test", action="store_true", default=True, help="Use test data instead of val data")
    parser.add_argument("--encoder_name", type=str, default="/home/wy3/zjc_data/PyProject/zero-shot-video-to-text/loc_openai/clip-vit-base-patch32",
                        help="Encoder name as found of HuggingFace or stored locally")
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--features_path", type=str, default="/home/wy3/zjc_data/zjc_500/MSVD-QA/key_images/")
    parser.add_argument("--features_type", type=str, default="")
