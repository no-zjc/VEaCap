...
def reweight_result_by_cos(result_list, cos_list):
    for i in range(len(cos_list)):
        rl = result_list[i]
        cl = cos_list[i]
        for j in range(len(rl)):
            if j == 0:
                rl[j] = (1 - label_smoothing_epsilon) * rl[j] + label_smoothing_epsilon * cl * tem_p
            else:
                rl[j] = (1 - label_smoothing_epsilon) * rl[j] + (label_smoothing_epsilon - (label_smoothing_epsilon * cl * tem_p)) / (classification_type - 1)
    return result_list
