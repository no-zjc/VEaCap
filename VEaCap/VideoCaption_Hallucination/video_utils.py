...
class TrainDataset(Dataset):
    def __init__(self, df, features_path, tokenizer, rag=False, template_path=None, k=None, max_caption_length=25, features_type=None):
        self.df = df
    ...
    def __getitem__(self, idx):
        ...
        if self.features is not None:
            output_tensor= VideoUtils.get_combine_features(self.features, self.df['fts_key'][idx])
        else:
            output_tensor = VideoUtils.get_all_features_by_json_files(self.features_type, self.df['fts_key'][idx], self.features_path)
            action_fts_o, video_obj_fts_m = None, None
...
class VideoUtils:
    ...
    def get_all_features(features, fts_key, video_path=None):
        ...
        output_tensor, object_fts_o, action_fts_o, video_obj_fts_m = VideoUtils.get_triplet_features(features, fts_key, video_path=None)
        output_tensor, action_fts_o = torch.cat([output_tensor], dim=0), torch.cat([action_fts_o], dim=0)
        if video_obj_fts_m is not None:
            video_obj_fts_m = torch.cat([video_obj_fts_m], dim=0)
        return output_tensor, action_fts_o, video_obj_fts_m

    @staticmethod
    def get_all_features_by_json_files(features_type, fts_key, feature_path, video_path=None):
        # load precomputed features
        # 最大的特征维度，特征默认max_fts_dim*512
        max_fts_dim = 80
        padding = torch.tensor(0)
        ...
        if video_path is not None:
            ...
            # video_fts = features["video_fts"][fts_key][()]
            fts_path = feature_path + os.sep + fts_key
            fts_path_s = File_Tools.find_and_sort_files_by_prefix(fts_path, features_type)
            video_fts = None
            for fps in fts_path_s:
                try:
                    frame_fts = torch.tensor(File_Tools.load_json_data(fps)).to(torch.float32)
                    if frame_fts.size(-1) >= 4096:
                        # frame_fts = frame_fts.squeeze(0)
                        frame_fts_max_length_1 = 80
                        frame_fts_max_length_2 = 4096
                        if frame_fts.size(1) > frame_fts_max_length_1:
                            frame_fts = frame_fts[:, :frame_fts_max_length_1, :frame_fts_max_length_2]
                        elif frame_fts.size(1) < frame_fts_max_length_1:
                            padding_length = (0, 0, 0, frame_fts_max_length_1 - frame_fts.size(1))
                            frame_fts = torch.nn.functional.pad(frame_fts, padding_length, value=padding)
                            frame_fts = frame_fts[:, :, :frame_fts_max_length_2]
                    if video_fts is None:
                        video_fts = torch.cat([frame_fts], dim=0)
                    else:
                        video_fts = torch.cat([video_fts, frame_fts], dim=0)
                except:
                    print('path', fps, ' data format error!')
                    continue
        # 获取张量的维度数量 n
        n = video_fts.ndimension()

        # 创建一个长度为 2n 的数组，通常用于指定每个维度的前后填充量
        # 例如我们用0初始化这个数组，方便后续操作
        pad = [0] * (2 * n)
        pad[-1] = max_fts_dim - video_fts.shape[0]
        output_tensor = torch.nn.functional.pad(video_fts, pad, value=padding)
    ...
    def create_feature_hdf5(self, video_path, h5file_save_path, videoUtils=None):
        """
    ...
    def load_data_for_VATEX(self, annot_path, split_dict_file, caps_path=None, mapping_path=None):
        annotations = json.load(open(annot_path))
        mapping_dict = File_Tools.get_vatex_mapping_dict()
        split_dict = File_Tools.load_json_data(split_dict_file)
        if caps_path is not None:
            retrieved_caps = json.load(open(caps_path))
        ...
        for i, (key, value) in enumerate(annotations.items()):
            file_name = key + ".mp4"
            vid_int = int(mapping_dict[key].split("video")[1])
            ...
            if key in split_dict['train']:
                for item in value:
                    samples.append({'file_name': file_name, 'vid': vid_int, 'caps': caps, 'text': item, 'fts_key': key})
                data['train'] += samples
            elif key in split_dict['val']:
                samples.append({'file_name': file_name, 'vid': vid_int, 'caps': caps, 'text': value[0], 'fts_key': key})
                data['val'] += samples
            elif key in split_dict['test']:
                samples.append({'file_name': file_name, 'vid': vid_int, 'caps': caps, 'text': value[0], 'fts_key': key})
                data['test'] += samples
        return data
    ...
    @staticmethod
    def get_one_video_images_by_h5(fts_path, videos_path, output_path, video_name):
        """
        根据特征文件中的关键帧信息提取视频文件中的对应帧并保存为jpg格式
        input：keyframe[]
        output: imagedatasets
