...
class File_Tools():
    ...
    @staticmethod
    def get_msvd_mapping_dict_by_vid(mapping_file="/home/wy3/zjc_data/datasets/MSVD-QA/youtube_mapping.txt"):
        # 打开文件
        mapping_dict = {}
        with open(mapping_file, 'r') as file:
            # 逐行读取并按空格拆分后输出
            for line in file.readlines():
                words = line.split()  # 使用 split 方法按空格拆分字符串，默认按空格拆分
                mapping_dict.update({words[1]: words[0]})
        # print(mapping_dict)
        return mapping_dict

    @staticmethod
    def get_vatex_mapping_dict(mapping_file="/home/wy3/zjc_data/datasets/VATEX/vatex_mapping.txt"):
        # 打开文件
        mapping_dict = {}
        with open(mapping_file, 'r') as file:
            # 逐行读取并按空格拆分后输出
    ...
    @staticmethod
    def get_vatex_mapping_dict_by_vid(mapping_file="/home/wy3/zjc_data/datasets/VATEX/vatex_mapping.txt"):
        # 打开文件
        mapping_dict = {}
        with open(mapping_file, 'r') as file:
            # 逐行读取并按空格拆分后输出
    ...
    @staticmethod
    def find_files_with_suffix(directory, suffix='.mp4'):
        files_info = []
        for root, _, files in os.walk(directory):
            for file in files:
                if file.endswith(suffix):
                    ...
                    files_info.append([full_path, dir_path, file_name, file_name_no_suffix])
        return files_info

    @staticmethod
    def find_and_sort_files_by_prefix(directory, prefix):
        # 查找所有以 blip2_output_frame_ 开头的文件
        pattern = re.compile(prefix + r'(\d+)\.json')
        files = []
        ...
        for filename in os.listdir(directory):
            ...
            if match:
                ...
                # 保存文件路径和数字
                files.append((os.path.join(directory, filename), num))
...
if __name__ == '__main__':
    ...
    md = File_Tools.get_vatex_mapping_dict_by_vid()
    print(len(md))
    tail = '.mp4'
    exist_video = 0
    not_exist_video = 0
    rp = '/home/wy3/zjc_data/PyProject/CoCap/dataset/vatex/videos_240_h264_keyint_60/'
    for k, v in md.items():
        if not os.path.exists(rp + k + tail):
            if os.path.exists(rp + v + tail):
                print(k, 'is changed')
                exist_video += 1
                continue
            else:
                ...
                not_exist_video += 1
                continue
        File_Tools.file_rename(rp + k + tail, v + tail)
        exist_video += 1

    print('exist_video:', str(exist_video))
    print('not_exist_video:', str(not_exist_video))
